...
namespace Slide15
{
	...
	public class Slide15
	{
					...
					case ConsoleKey.Enter:
						...
						if(isClear)
						{
							...
							Console.Clear();
							{
								...
								Console.Write("åªç›ÇÃÉ^ÉCÉÄ: {0}ïb",stopwatch.Elapsed.Hours*3600+stopwatch.Elapsed.Minutes*60+stopwatch.Elapsed.Seconds);
							}
						}
						for(int i=0;i<Data[selected-3].Length-1 || i<5;i++)
						{
							if(0<=i && i<= Data[selected-3].Length-1)
							{
								...
								Console.Write("{0}à : {1}ïb ({2})",i+1,Data[selected-3][i].data,Data[selected-3][i].name);
		...
		public static void Main()
		{
			...
			Console.WriteLine("ÉâÉìÉLÉìÉOÇ…égÇ§ÇÃÇ≈ÅAñºëOÇì¸óÕÇµÇƒÇ≠ÇæÇ≥Ç¢ÅB");
